...
public class JdbcWorkflowTenantRepository implements WorkflowTenantRepository {
    ...
    @Override
    public PageSlice<WorkflowTenant> page(int pageNum, int pageSize, String keyword, Boolean enabled) {
        ...
        List<WorkflowTenant> records = jdbcTemplate.query(
                ...
                this::mapTenant,
                arguments.toArray());
        return new PageSlice<>(total == null ? 0 : total, pageNum, pageSize, records);
    }
    ...
    public void update(WorkflowTenant tenant) {
        jdbcTemplate.update("""
                ...
                SET tenant_name = ?, description = ?, enabled = ?,
                    updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
                """, tenant.tenantName(), tenant.description(), tenant.enabled() ? 1 : 0, tenant.id());
    }
